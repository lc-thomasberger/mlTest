define([
  ...
  var LanguagePickerModel = Backbone.Model.extend({
    ...
    // do we need this? course data is populated directly from the new json
    resetCourseProgress: function () {
      Adapt.components.each(function(componentModel) {
        componentModel.reset("hard", true);
        if (componentModel.get("_isQuestionType")) componentModel.set("_userAnswer", undefined);
      });
    },
    ...
    checkDataIsLoaded: function () {
      if (Adapt.contentObjects.models.length > 0 && Adapt.articles.models.length > 0 && Adapt.blocks.models.length > 0 && Adapt.components.models.length > 0) {
        ...
        // move to AdaptModel
        // this.listenToOnce(Adapt, 'app:dataReLoaded', this.setupModel);
        // trigger from AdaptCollection and CourseModel after fetch
        ...
        */
        ...
        // update app.js to call this by triggering an event
